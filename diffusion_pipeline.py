...
class DiffisionPipeline(object):
    ...
    def generate(
        ...
        rows = torch.arange(seq_len, device=self.device).unsqueeze(1)
        cols = torch.arange(seq_len, device=self.device).unsqueeze(0)
        diff = rows - cols
        attn_mask = (diff < -self.seq_len) | (diff >= self.seq_len)
        ...
        n = 1
        z = seq_x.expand(n, -1, -1).to(self.device)
        c = seq_c.expand(n, -1, -1).to(self.device)
        y = class_vector.expand(n, -1).to(self.device)
        y_null = unk_class_vector.expand(n, -1).to(self.device)
        ...
        def to_positions(samples):
            ...
            samples += 1
            samples /= 2
            samples *= torch.tensor((512, 384), device=self.device).view(1, 2, 1)
            return samples.cpu()
