    ...
    def _batch_thread(self):
        while not self.shutdown_flag.is_set():
            ...
            try:
                ...
                # Avoid calling empty_cache on every batch unless shapes changed to prevent CUDA sync stalls
                if torch.cuda.is_available():
                    current_shapes = None
                    if 'model_kwargs' in locals() and isinstance(model_kwargs, dict):
                        current_shapes = {k: v.shape for k, v in model_kwargs.items() if isinstance(v, torch.Tensor)}
                    if getattr(self, 'last_shapes', None) != current_shapes:
                        self.last_shapes = current_shapes
