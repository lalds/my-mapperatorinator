...
class SDPAAttention(nn.Module):
    """
    A custom MultiheadAttention replacement using F.scaled_dot_product_attention.
    Fully state_dict compatible with nn.MultiheadAttention.
    """
    ...
    def forward(self, query, key, value, need_weights=False, attn_mask=None, key_padding_mask=None):
        # input is batch_first=True, shape: [B, T, D]
        B, T, D = query.shape
        
        # Project using in_proj_weight and in_proj_bias
        ...
        # Invert the boolean mask if provided:
        # nn.MultiheadAttention uses: True = masked (do not attend), False = attend.
        # F.scaled_dot_product_attention uses: True = attend, False = masked (do not attend).
...
class FirstLayer(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # Check if c is already cached
        if self._cached_c is c and self._cached_projected_c is not None:
            projected_c = self._cached_projected_c
        else:
            # Extract the part of weights corresponding to context_size
            linear_layer = self.mlp[0]
            w_c = linear_layer.weight[:, -c.shape[-1]:]
            
            # Project c
            projected_c = F.linear(c, w_c, bias=None)
            
            # Cache it
            self._cached_c = c
            self._cached_projected_c = projected_c
            
        # Project x_freq using the first part of the weights, and add bias + projected_c
        linear_layer = self.mlp[0]
        x_features = x_freq.shape[-1]
        w_x = linear_layer.weight[:, :x_features]
        bias = linear_layer.bias
        
        xc_emb = F.linear(x_freq, w_x, bias) + projected_c
        return xc_emb
